...
export function lottery(data, MIN_LOTTERY_COUNT, MIN_ITEM_COUNT, MIN_PROBABILITY, f) {
    ...
    for(let i = 0; i < lotteryCount; i++) {
        let maxId = 0;
        results.forEach((r) => {
            maxId += r.probability ?? MIN_PROBABILITY;
        });
        console.log(results, maxId);
        ...
        while(total < rand) {
            lotteryId++;
            total += results[lotteryId]?.probability ?? MIN_PROBABILITY;
            console.log(total, lotteryId);
        }
        console.log(lotteryId);
        const result = results[lotteryId];
        const newResults = [];
        ...
        if(result?.description) {
            ...
            const descriptionList = result?.description.split("\n");
            if (descriptionList) {
