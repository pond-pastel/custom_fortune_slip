...
function changeName(ele) {
    ...
    switch(attribute) {
        ...
        case "item-probability":
            if(!isNaN(Number(ele.value)) && Number(ele.value) >= MIN_PROBABILITY) {
                data.insert.items[index].probability = Number(ele.value);
            }
            break;
...
function nameValidation(ele) {
    ...
    switch(attribute) {
        ...
        case "item-probability":
            if(isNaN(Number(ele.value)) || Number(ele.value) < MIN_PROBABILITY) {
                const itemProbabilityEles = alertEles.filter((a) => a.classList.contains("item-probability") && a.dataset.id === ele.dataset.id);
                showAlertEles.push(...itemProbabilityEles);
            }
            break;
...
window.onload = function() {
    ...
    addItemButtonEle.addEventListener("click", () => {
        addItem(data, MIN_ITEM_COUNT, MIN_PROBABILITY, {
    ...
    addSetButtonEle.addEventListener("click", () => {
        addSet(data, INVALID_STRING, MIN_ITEM_COUNT, MIN_PROBABILITY, {
            nameValidation: nameValidation,
            supplementTheData: supplementTheData,
            saveData: saveData,
    ...
    lotteryButtonEle.addEventListener("click", () => {
        lottery(data, MIN_LOTTERY_COUNT, MIN_ITEM_COUNT, MIN_PROBABILITY, {
            nameValidation: nameValidation
        });
    });
    const editSetButtonEle = document.getElementById("edit-set-button");
    editSetButtonEle.addEventListener("click", () => {
        editSet(data, MIN_ITEM_COUNT, MIN_PROBABILITY, {
            nameValidation: nameValidation,
            supplementTheData: supplementTheData,
            applyAddItemList: applyAddItemList,
    ...
    deleteSetButtonEle.addEventListener("click", () => {
        deleteSet(data, MIN_PROBABILITY, {
            nameValidation: nameValidation,
            showAddOrEdit: showAddOrEdit,
            saveData: saveData,
            applyLotteryCountMax: applyLotteryCountMax
