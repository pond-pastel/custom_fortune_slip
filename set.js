export function addSet(data, INVALID_STRING, MIN_ITEM_COUNT, MIN_PROBABILITY, f) {
    console.log(data);
    ...
    judgeItems.forEach((i) => {
        if(Number(i.probability) < MIN_PROBABILITY) {
    ...
    f.saveData();
    f.applyAddItemList(data, MIN_ITEM_COUNT, MIN_PROBABILITY, {
        nameValidation: f.nameValidation
    });
    applySets(data);
    f.showAddOrEdit();
    applySetContents(data, MIN_PROBABILITY, {
        applyLotteryCountMax: f.applyLotteryCountMax,
        saveData: f.saveData,
        nameValidation: f.nameValidation
    });
    const step5Ele = document.getElementById("step-5");
    const scrollY = window.scrollY;
    const pos = step5Ele.getBoundingClientRect().y + scrollY;
    window.scrollTo({
        top: pos,
        behavior: "smooth"
    });
...
export function editSet(data, MIN_ITEM_COUNT, MIN_PROBABILITY, f) {
...
export function deleteSet(data, MIN_PROBABILITY, f) {
    const setSelectorEle = document.getElementById("set-selector");
    const selectedId = setSelectorEle.value;
    if(selectedId === "") {
...
export function applySetContents(data, MIN_PROBABILITY, f) {
    const setSelectorEle = document.getElementById("set-selector");
    ...
    (set.items ?? []).forEach((item, index) => {
        ...
        checkbox.checked = item.use ?? true;
        checkbox.addEventListener("change", () => {
            f.nameValidation(lotteryCountEle);
            item.use = checkbox.checked;
            f.saveData();
        });
        itemEle.append(checkbox, (item.name ?? "読み込みエラー"));
        group.append(itemEle);
        const itemProbability = document.createElement("div");
        itemProbability.textContent = `出やすさ: ${item.probability ?? MIN_PROBABILITY}`;
        group.append(itemProbability);
        if(item.description) {
